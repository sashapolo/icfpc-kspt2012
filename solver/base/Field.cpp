...
Field::Field(const std::string& mapFileName) {
	std::ifstream file;
	char* file_buf;
	int file_size;
	...
	if (!file.is_open()) {
		LOGERROR("Can't load map from \"%s\": can't open file",
				mapFileName.c_str());
		throw FieldParseException();
	}
	...
	file_size = file.tellg();
	if (file_size == 0) {
		LOGERROR("Can't load map from \"%s\": file is empty",
				mapFileName.c_str());
		file.close();
		throw FieldParseException();
	}
	...
	try {
		...
		LOGERROR("Can't load map from \"%s\": map is incorrect",
				mapFileName.c_str());
		delete[] file_buf;
		throw e;
	}
